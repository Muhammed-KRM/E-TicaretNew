...
public class UserManager : IUserService
{
    ...
    public async Task<UserProfileDto> GetProfileAsync(Guid userId)
    {
        try
        {
            ...
            return new UserProfileDto
            {
                ...
                NotificationSettings = new NotificationSettingsDto { EmailNotifications = user.EmailNotifications, MarketingEmails = user.MarketingEmails, SmsNotifications = false }
            };
        }
    ...
    public async Task UpdateNotificationSettingsAsync(Guid userId, NotificationSettingsDto dto)
    {
        ...
        {
            var user = await _userRepo.GetByIdAsync(userId) ?? throw new NotFoundException("Kullanıcı", userId);
            user.EmailNotifications = dto.EmailNotifications; user.MarketingEmails = dto.MarketingEmails;
            _userRepo.Update(user); await _userRepo.SaveChangesAsync();
        }
        catch (NotFoundException) { throw; }
