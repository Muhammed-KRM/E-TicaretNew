...
namespace ETicaret.Business.Infrastructure.Email;
...
public class SmtpEmailService : IEmailService
{
    ...
    public async Task SendEmailAsync(string to, string subject, string htmlBody)
    {
        ...
        message.From.Add(new MailboxAddress(
            smtpSettings["SenderName"] ?? "ETicaret.com",
    ...
    public async Task SendTemplatedEmailAsync(string to, string templateName, Dictionary<string, string> replacements)
    {
        ...
        {
            case "WelcomeEmail":
                subject = "ETicaret.com'a Hoş Geldiniz! 🎓";
                htmlBody = $@"
                    <div style='font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;'>
                        <h1 style='color: #6366f1;'>Hoş Geldiniz, {replacements.GetValueOrDefault("FullName", "")}!</h1>
                        <p>ETicaret.com'a başarıyla kayıt oldunuz.</p>
                        <p>Artık binlerce öğretmen ve öğrenci arasında aradığınız dersi bulabilir veya ders verebilirsiniz.</p>
                        <a href='https://ETicaret.com/panel' style='display:inline-block; padding:12px 24px; background:#6366f1; color:white; border-radius:8px; text-decoration:none;'>Panelime Git</a>
                    </div>";
                break;
            ...
            default:
                subject = "ETicaret.com Bilgilendirme";
