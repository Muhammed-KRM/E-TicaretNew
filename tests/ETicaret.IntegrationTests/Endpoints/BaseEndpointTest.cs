using ETicaret.IntegrationTests.Setup;
...
namespace ETicaret.IntegrationTests.Endpoints;
...
public abstract class BaseEndpointTest : IClassFixture<CustomWebApplicationFactory>
{
    ...
    {
        Guid userId;
        var role = ETicaret.Data.Enums.UserRole.User;

        using (var scope = _factory.Services.CreateScope())
        {
             var db = scope.ServiceProvider.GetRequiredService<ETicaret.Data.Context.AppDbContext>();
             var user = db.Users.FirstOrDefault(u => u.Email == email);
             if (user == null) 
             {
                 user = new ETicaret.Data.Entities.User
                 {
                     Id = Guid.NewGuid(),
                     Email = email,
                     FullName = fullName,
                     PasswordHash = ETicaret.Business.Helpers.PasswordHasher.Hash(password),
                     Role = role,
                     IsActive = true
                 };
                 db.Users.Add(user);
        ...
        var token = ETicaret.API.Helpers.JwtHelper.GenerateToken(userId, email, fullName, role.ToString(), config);
    ...
    {
        string email = "admin_tester_e2e@ETicaret.com";
        ...
        using (var scope = _factory.Services.CreateScope())
        {
             ...
             {
                 user.Role = ETicaret.Data.Enums.UserRole.Admin;
             }
             db.SaveChanges();
             userId = user.Id;
        }
        ...
        var config = _factory.Services.GetRequiredService<Microsoft.Extensions.Configuration.IConfiguration>();
        var token = ETicaret.API.Helpers.JwtHelper.GenerateToken(userId, email, "Admin Tester", ETicaret.Data.Enums.UserRole.Admin.ToString(), config);
        _client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
    }
